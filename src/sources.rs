...
pub(crate) fn collect_agents(
    ...
    // Pane renames are also absent from agent rows. `label` is only present
    // once a pane has been renamed.
    let pane_json = herdr_json(["pane", "list"]).unwrap_or(Value::Null);
    agents_from_json(
        &agent_json,
        &tab_json,
...
fn labels_by_id<'a>(json: &'a Value, list_key: &str, id_key: &str) -> HashMap<&'a str, &'a str> {
...
fn agents_from_json(
    ...
    let workspace_labels: HashMap<&str, &str> = workspaces
        ...
        .collect();
    let tab_labels = labels_by_id(tab_json, "tabs", "tab_id");
    let pane_labels = labels_by_id(pane_json, "panes", "pane_id");
    let mut entries = Vec::new();
        ...
        .and_then(|v| v.as_array())
    {
        for p in agents {
            ...
            let pane_label = pane_labels.get(pane).copied();
            let mut parts = vec![display_agent];
            if let Some(name) = pane_label {
                parts.push(name);
            }
            if let Some(name) = tab_label {
                parts.push(name);
            }
            if let Some(task) = task_title {
                parts.push(task);
            }
            parts.push(workspace_label);
            parts.push(&dir);
...
mod tests {
    ...
    fn parses_herdr_workspace_and_agent_list_json() {
        ...
        let agents = agents_from_json(&agent_json, &Value::Null, &Value::Null, &entries, &[], true);
    ...
    fn unnamed_and_blank_named_agents_fall_back_to_the_agent_kind() {
        ...
        let agents = agents_from_json(&agent_json, &Value::Null, &Value::Null, &[], &[], true);
    ...
    fn agent_display_metadata_is_visible_and_fuzzy_searchable() {
        ...
        let agents = agents_from_json(&input, &Value::Null, &Value::Null, &[], &[], true);
        let agent = &agents[0];
    ...
    fn metadata_can_be_disabled_without_losing_agent_identity() {
        ...
        let agents = agents_from_json(&input, &Value::Null, &Value::Null, &[], &[], false);
    ...
    fn renamed_tab_label_finds_every_agent_on_that_tab() {
        ...
        let agents = agents_from_json(&agents_json, &tabs_json, &Value::Null, &[], &[], true);
    ...
    #[test]
    fn renamed_pane_label_is_visible_and_searchable() {
        let agents_json = serde_json::json!({"result":{"agents":[
            {"agent":"pi","pane_id":"w5B:p1","tab_id":"w5B:t1","cwd":"/tmp/backend","workspace_id":"w5B","display_agent":"pi"},
            {"agent":"pi","pane_id":"w5B:p2","tab_id":"w5B:t1","cwd":"/tmp/backend","workspace_id":"w5B","display_agent":"pi"}
        ]}});
        let panes_json = serde_json::json!({"result":{"panes":[
            {"pane_id":"w5B:p1","label":"valkey"},
            {"pane_id":"w5B:p2","label":"   "},
            {"pane_id":"w5B:p3"}
        ]}});
        let agents = agents_from_json(&agents_json, &Value::Null, &panes_json, &[], &[], true);
        assert!(agents[0].title.contains("valkey"));
        assert!(agents[0].search_terms.contains(&"valkey".to_string()));
        assert!(!agents[1].title.contains("valkey"));
        assert!(!agents[1].search_terms.iter().any(|term| term == "   "));
        let mut scorer = crate::matcher::Scorer::new("nucleo", "valkey");
        assert!(scorer.score(&agents[0].haystack()).is_some());
        assert!(scorer.score(&agents[1].haystack()).is_none());
    }
